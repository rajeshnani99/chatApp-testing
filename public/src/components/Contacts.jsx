...
import allchats from "../utils/APIRoutes";
...
export default function Contacts({
	...
	const [groupchat1, setGroupChat1] = useState(undefined);
	...
	}, [currentUser, groupchat1]);

	const changeCurrentChat = (index, contact) => {
		setCurrentSelected(index);
		changeChat(contact);
	};
	const changeGroupChat = (index, groupchat) => {
		setGroupChat1(index);
		changeChat(groupchat);
		console.log("check:" + groupchat);
	};
			...
			{currentUserImage && currentUserName && (
				<Container>
					...
					<div className="contacts">
						{groupchat.map((user, index) => {
							return (
								<div
									style={{ color: "white" }}
									key={user._id}
									onClick={() => changeGroupChat(index, groupchat)}>
